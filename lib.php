 ...
 * of the new instance.
 *
 * @param stdClass $streamline Submitted data from the form in mod_form.php
 * @param mod_streamline_mod_form $mform The form instance itself (if needed)
...
function streamline_add_instance(stdClass $streamline, mod_streamline_mod_form $mform = null) {
    ...
    $streamline->id = $DB->insert_record('streamline', $streamline);

    streamline_grade_item_update($streamline);
 ...
 *
 * @param stdClass $streamline An object from the form in mod_form.php
 * @param mod_streamline_mod_form $mform The form instance itself (if needed)
...
function streamline_update_instance(stdClass $streamline, mod_streamline_mod_form $mform = null) {
    ...
    $result = $DB->update_record('streamline', $streamline);
...
function streamline_delete_instance($id) {
    ...
    streamline_grade_item_delete($streamline);
 ...
 * @param stdClass $streamline The streamline instance record
...
/**
 * Prints single activity item prepared by {@link streamline_get_recent_mod_activity()}
 *
...
/**
 * Creates or updates grade item for the given streamline instance
 *
 ...
 * @return void
 */
function streamline_grade_item_update(stdClass $streamline, $reset=false) {
    global $CFG;
    ...
    if ($streamline->grade > 0) {
        $item['gradetype'] = GRADE_TYPE_VALUE;
        $item['grademax']  = $streamline->grade;
        $item['grademin']  = 0;
    } else if ($streamline->grade < 0) {
        $item['gradetype'] = GRADE_TYPE_SCALE;
        $item['scaleid']   = -$streamline->grade;
    } else {
    ...
    grade_update('mod/streamline', $streamline->course, 'mod', 'streamline',
            $streamline->id, 0, null, $item);
}

/**
 * Delete grade item for given streamline instance
 *
 * @param stdClass $streamline instance object
 * @return grade_item
 */
function streamline_grade_item_delete($streamline) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    return grade_update('mod/streamline', $streamline->course, 'mod', 'streamline',
            $streamline->id, 0, null, array('deleted' => 1));
}
 ...
 * @param stdClass $module current streamline instance record
 ...
 * @param navigation_node $streamlinenode streamline administration node
 */
